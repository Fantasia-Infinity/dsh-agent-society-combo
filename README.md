...
`1ef9c1fa9afbea78c5537bfc8b6d1c27d598798e`）；AgentSociety 插件为
`@agent-society/dsh-agent-society`。Combo 会分别校验 npm 与 source 模式的 dsh
版本；两种模式的差异只通过显式 `--source` 选择，不会静默混用。

注意：上游 fork 的源码适配已前移到 `454f46f35983783f9e74d4edf7921bcc6104febf`
并用于显式 `combo install --source`；默认 npm 模式使用该 fork 已包含的
`0.1.5-rc.1` 发布提交。实验性的 Agent Teams 保持上游默认关闭，不加入 Combo profile。

dsh-TUI `0.10.1` 已对齐 dsh `0.1.5-rc.1`。源码模式会初始化它新增的
`dsh-auth` / `dsh-std` 子模块，并使用 Combo 的 sibling-checkout build 与独立
source launcher patch；旧的 rc.7 / alpha API patch 已移除。该适配只覆盖 TUI 启动和
...
```bash
npm install -g @deepseek-ai/dsh@0.1.5-rc.1 @agent-society/agent-host
